...
$PAGE->set_url('/mod/eportfolio/index.php', ['id' => $id]);
...
$modulenameplural = get_string('modulenameplural', 'mod_eportfolio');
...
foreach ($eportfolios as $eportfolio) {
    if (!$eportfolio->visible) {
        $link = html_writer::link(
                new moodle_url('/mod/eportfolio/view.php', ['id' => $eportfolio->coursemodule]),
                format_string($eportfolio->name, true),
                ['class' => 'dimmed']);
    } else {
        $link = html_writer::link(
                new moodle_url('/mod/eportfolio/view.php', ['id' => $eportfolio->coursemodule]),
    ...
    if ($course->format == 'weeks' || $course->format == 'topics') {
        $table->data[] = [$eportfolio->section, $link];
    } else {
        $table->data[] = [$link];
    }
}
